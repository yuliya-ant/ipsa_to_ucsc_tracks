...
header <- 'track name="chx_cassette" description="chx_cassette_exons, PSI = (i1 + i2) / (2e + i1 + i2)" visibility=2 color=103,137,33 itemRgb="on" useScore=1'

header <- 'track name="srsf3_kd_cassette" description="srsf3_kd_cassette_exons, PSI = (i1 + i2) / (2e + i1 + i2)" visibility=2 color=103,137,33 itemRgb="on" useScore=1'
cat(header, '\n', file=path_out, append = FALSE)
write.table(ce_bed1,
            file = path_out, append = TRUE, sep=' ', 
            quote = FALSE, row.names = FALSE, col.names = FALSE)

header <- 'track name="rbfox2_kd_cassette" description="rbfox2_kd_cassette_exons, PSI = (i1 + i2) / (2e + i1 + i2)" visibility=2 color=71,48,120 itemRgb="on" useScore=1'
